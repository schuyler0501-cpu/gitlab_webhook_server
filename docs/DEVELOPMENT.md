# 开发规范文档

## 📋 目录结构

```
gitlab_webhook_server/
├── cmd/                    # 应用入口
│   └── server/            # 服务器主程序
│       └── main.go
├── internal/              # 内部包（不对外暴露）
│   ├── config/           # 配置管理
│   ├── handler/          # HTTP 处理器
│   ├── model/            # 数据模型
│   ├── router/           # 路由定义
│   ├── service/          # 业务逻辑层
│   │   └── commit/       # 提交相关服务
│   └── logger/           # 日志工具
├── pkg/                   # 可对外暴露的包（如果有）
├── test/                  # 测试数据
├── docs/                  # 文档
├── bin/                   # 编译输出（自动生成）
└── go.mod                 # Go 模块定义
```

## 🎯 代码规范

### 1. 命名规范

- **包命名**: 使用小写字母，简短有意义，如 `handler`, `service`
- **文件命名**: 使用小写字母和下划线，如 `webhook_handler.go`
- **类型命名**: 使用 PascalCase，如 `WebhookHandler`, `CommitRecord`
- **函数/变量命名**: 使用 camelCase，如 `handleWebhook`, `commitService`
- **常量命名**: 使用 PascalCase 或 UPPER_SNAKE_CASE，如 `MaxRetries`, `DEFAULT_PORT`

### 2. 代码组织

- **分层架构**: Handler → Service → Repository/Data Layer
- **单一职责**: 每个包/文件只负责一个功能
- **接口优先**: 使用接口定义服务契约，便于测试和扩展

### 3. 注释规范

```go
// HandleWebhook 处理 GitLab Webhook 请求
// 验证 token，解析请求体，并调用服务层处理业务逻辑
func (h *WebhookHandler) HandleWebhook(c *gin.Context) {
    // 实现代码
}
```

### 4. 错误处理

- 始终检查错误并适当处理
- 使用 `fmt.Errorf` 或 `errors.Wrap` 添加上下文
- 记录错误日志，不要静默忽略

```go
if err != nil {
    logger.Error("操作失败", zap.Error(err))
    return fmt.Errorf("操作失败: %w", err)
}
```

## 🛠️ 开发工作流

### 1. 安装依赖

```bash
# 下载依赖
go mod download

# 整理依赖
go mod tidy
```

### 2. 开发模式

```bash
# 使用 Air 热重载（推荐）
make dev

# 或直接运行
go run cmd/server/main.go
```

### 3. 代码检查

```bash
# 运行 linter
make lint

# 格式化代码
make fmt
```

### 4. 构建项目

```bash
# 构建
make build

# 运行
make run
```

### 5. 运行测试

```bash
# 运行所有测试
make test

# 运行特定包的测试
go test ./internal/handler/...
```

## 📝 Git 提交规范

建议使用以下格式的提交信息：

```
<type>(<scope>): <subject>

<body>

<footer>
```

**Type 类型**:
- `feat`: 新功能
- `fix`: 修复 bug
- `docs`: 文档更新
- `style`: 代码格式调整
- `refactor`: 代码重构
- `test`: 测试相关
- `chore`: 构建/工具相关

**示例**:
```
feat(webhook): 添加 GitLab Push 事件处理

实现了 Push Hook 事件的接收和处理逻辑，
包括提交记录的解析和存储。

Closes #123
```

## 🤖 AI 辅助开发提示

### 与 AI 协作的最佳实践

1. **清晰的上下文**: 在提问时提供足够的上下文信息
2. **具体的问题**: 明确说明你想要实现的功能
3. **代码引用**: 使用文件路径和行号引用现有代码
4. **迭代开发**: 分步骤实现功能，逐步完善

### 示例提示词

✅ **好的提示**:
```
在 internal/service/commit/commit_service.go 中添加一个方法，
根据作者邮箱和时间范围查询提交记录，并返回统计信息。
```

❌ **不好的提示**:
```
帮我写个查询功能
```

## 🔍 调试技巧

1. **使用日志**: 使用 `zap` 日志库记录关键信息
2. **单元测试**: 为每个功能编写测试
3. **接口测试**: 使用 Postman 或 curl 测试 API

## 📚 相关资源

- [Go 官方文档](https://go.dev/doc/)
- [Gin 框架文档](https://gin-gonic.com/docs/)
- [GitLab Webhook 文档](https://docs.gitlab.com/ee/user/project/integrations/webhooks.html)
- [Uber Zap 日志库](https://github.com/uber-go/zap)

## 🛠️ 推荐工具

- **Air**: 热重载工具，`go install github.com/air-verse/air@latest`
- **golangci-lint**: 代码检查工具，`go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest`
- **goimports**: 自动导入管理，`go install golang.org/x/tools/cmd/goimports@latest`
